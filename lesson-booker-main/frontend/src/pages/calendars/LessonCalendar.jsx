  ...
  const fetchEventsforSelectedTeacher = async () => {
    try {
      ...
      const events = data.docs.map((doc) => {
        ...
        // Ensure start and end are converted from Firestore Timestamps to JavaScript Dates
        return {
          ...eventData,
          id: doc.id,
          start: eventData.start?.toDate(), // Convert Firestore Timestamp to Date
          end: eventData.end?.toDate(),     // Convert Firestore Timestamp to Date
        };
      });
...
//     console.log("Fetched events for teacher:", events);
//   } catch (error) {
//     console.error("Error fetched for teacher events", error);
